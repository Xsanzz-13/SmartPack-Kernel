...
public class CPUVoltageFragment extends RecyclerViewFragment {
    ...
    protected void addItems(List<RecyclerViewItem> items) {
        ...
        List<String> clusters = mVoltage.getClusters();
        if (clusters.size() > 1) {
            SelectView clusterView = new SelectView();
            clusterView.setTitle("CPU Cluster");
            clusterView.setItems(clusters);
            clusterView.setItem(mVoltage.getCluster());
            clusterView.setOnItemSelected((selectView, position, item) -> {
                if (position != mVoltage.getCluster()) {
                    mVoltage.setCluster(position);
                    rebuildItems();
                }
            });
            items.add(clusterView);
        }
